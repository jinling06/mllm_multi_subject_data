...
def parse_thread(samples, group_data, version_dir, save_path, index_prefix, keyword):
    ...
    for sample_name in tqdm(samples, total=len(samples), desc="sample"):
        ...
        for i in range(len(html_data_list)):
            ...
            temp_dic = {
                "id": f"mllm_data_3_v3_{index_prefix}_{index}",
                "category": keyword,
                "keyword": keyword,
...
if __name__ == '__main__':
    # 只有 政治和历史
    data_dir = 'MLLM_data_3_v3'
    json_save_dir = f'{data_dir}/common_ratio_json_file_with_formula'
    save_path = f'{data_dir}/train_1110.json'
    only_image_save_path = f'{data_dir}/train_1110_filter_with_image.json'

    # parse_mllm_data_3_except_chinese(data_dir, json_save_dir)

    filter_data(json_save_dir, save_path, source_data_name='mllm_3', filter_over_width_image=False)
