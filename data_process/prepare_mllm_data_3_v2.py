...
def parse_thread(samples, group_data, version_dir, save_path, index_prefix, keyword):
    ...
    for sample_name in tqdm(samples, total=len(samples), desc="sample"):
        ...
        for i in range(len(html_data_list)):
            ...
            temp_dic = {
                "id": f"mllm_data_3_v2_{index_prefix}_{index}",
                "category": keyword,
                "keyword": keyword,
...
def parse_mllm_data_3_except_chinese(data_dir, json_save_dir):
    ...
    for subject in os.listdir(data_dir):
        ...
        if subject not in ['CHEMISTRY_img', 'CHEMISTRY_img_v2', 'PHYSICS_img']:
            continue
...
if __name__ == '__main__':
    # 化学地理物理  全部 159k 其中公式图 91k
    data_dir = 'MLLM_data_3_v2'
    ...
    save_path = f'{data_dir}/train_1027_with_formula_filter.json'
